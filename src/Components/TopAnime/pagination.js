...
const Pagination = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/seasons/now?limit=5&page=${halaman}`)
    ...
    return (
        <div>
            ...
            <div className="gap-4 flex">
                <button onClick={() => setHalaman((prev) => prev - 1)} className="px-4 border-2 rounded cursor-pointer">Previous</button>
                <button onClick={() => setHalaman((prev) => prev + 1)} className="px-4 border-2 rounded cursor-pointer">Next</button>
            </div>
        </div>
    )
}
